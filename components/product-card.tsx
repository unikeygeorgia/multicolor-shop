...
import { fmt, hasTag, minPrice, prodImg, salePrice } from "@/lib/utils";
import { useStore } from "@/components/store-provider";
import { PlusIcon } from "@/components/icons";

export function ProductCard({ product: p }: { product: Product }) {
  const { brandById, quickAdd, settings } = useStore();
  const b = brandById(p.brand);
  const base = minPrice(p);
  const onSale = !!p.salePct && !settings.pricesHidden;
  const colors = (p.colors || []).slice(0, 5);
  const extra = (p.colors || []).length - colors.length;
  const sizes = p.sizes.slice(0, 4);
  const href = `/product?slug=${p.slug || p.id}`;
      ...
      <div className="info">
        ...
        {colors.length > 0 && (
          <div className="swatches">
            {colors.map((c, i) => (
              <span key={`${c.n}-${i}`} className="dot" style={{ background: c.h }} title={c.n} />
            ))}
        ...
        <div className="sizes">
          {sizes.map((s, i) => (
            <span key={`${s.l}-${i}`} className="size-pill">{s.l}</span>
          ))}
          {p.sizes.length > 4 && <span className="size-pill">+{p.sizes.length - 4}</span>}
        </div>
        {!settings.pricesHidden && (
          <div className="pricebar">
            {p.sizes.length > 1 && <span className="price-from">დან</span>}
            <span className={`price${onSale ? " sale" : ""}`}>{fmt(salePrice(p, base))}</span>
